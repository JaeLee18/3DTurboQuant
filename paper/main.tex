...
\begin{abstract}
We show that spherical harmonic (SH) coefficients in 3D Gaussian Splatting (3DGS) massively overfit, contributing up to 7\,dB more to training views than to test views across 21 standard benchmarks.
...
We show that SH coefficients in 3DGS massively overfit, contributing up to 7\,dB more to training views than test views---and that this overfitting gap makes compression effectively free on certain scenes.
...
\begin{enumerate}
\item \textbf{First systematic quantification of SH overfitting} in full-view 3DGS across 21 scenes from 4 benchmarks, revealing up to 7\,dB train/test gaps and the consistent band-level hierarchy $\R{1} < \R{2} < \R{3}$.
...
Initial measurements with only 5 test views produced spurious negative gaps for two scenes (\textit{bonsai} and \textit{room}) due to sampling variance, highlighting the importance of adequate evaluation views for stable gap estimation.
With 15 held-out views, \textit{bonsai} ($G{=}1.69\dB$) and \textit{room} ($G{=}0.38\dB$) both show positive gaps consistent with the overall trend.
...
Treehill presents an extreme case: $\R{3}$ is negative, indicating that zeroing band~3 \emph{improves} test PSNR by $0.11\dB$.
...
\begin{table*}[t]
\centering
\caption{SH overfitting analysis and compression results across 21 benchmark scenes. \textbf{Left:} $G$~=~train/test PSNR gap (dB); $\R{k}$~=~overfitting ratio for SH band $\ell{=}k$ (Eq.~\ref{eq:overfitting_ratio}). Higher $\R{k}$ indicates more overfitting. The hierarchy $\R{1} < \R{2} < \R{3}$ holds on average. \textbf{Right:} \ours{} compression ratio and quality changes ($\Delta$PSNR, $\Delta$SSIM, $\Delta$LPIPS) at the balanced operating point. Bold entries highlight effectively free compression ($|\Delta\text{PSNR}| \leq 0.05\dB$).}
...
& bicycle    & 5.23 & 1.86 & 6.29 & 3.94 & 9.9$\times$ & $-$0.21 & $-$.0195 & +.0125 \\
& bonsai     & 1.69 & $-$0.73 & 0.41 & 0.12 & 9.5$\times$ & $-$0.48 & $-$.0049 & +.0077 \\
& counter    & 0.53 & 1.12 & 1.69 & 1.39 & 9.2$\times$ & $-$0.27 & $-$.0064 & +.0080 \\
& flowers    & 2.38 & 2.18 & 2.78 & 2.82 & 9.5$\times$ & $-$0.11 & $-$.0114 & +.0091 \\
& garden     & 3.97 & 1.82 & 2.90 & 2.80 & 9.2$\times$ & \textbf{$-$0.05} & $-$.0066 & +.0076 \\
& kitchen    & 2.44 & 1.94 & 1.95 & 1.50 & 9.3$\times$ & $-$0.36 & $-$.0071 & +.0060 \\
& room       & 0.38 & 1.26 & 1.13 & 0.98 & 9.9$\times$ & $-$0.38 & $-$.0053 & +.0061 \\
& stump      & 5.46 & 5.09 & 10.78 & 7.41 & 9.6$\times$ & $-$0.41 & $-$.0326 & +.0212 \\
& treehill   & 1.88 & 8.16 & 2.74 & $-$3.48 & 9.9$\times$ & \textbf{$-$0.02} & $-$.0098 & +.0131 \\
\cmidrule{2-10}
& \textit{Average} & \textit{2.66} & \textit{2.52} & \textit{3.41} & \textit{1.94} & \textit{9.6$\times$} & \textit{$-$0.25} & \textit{$-$.0115} & \textit{+.0101} \\
\midrule
\multirow{2}{*}{\rotatebox{90}{\tiny T\&T}}
& truck      & 1.51 & 1.13 & 1.57 & 1.30 & 9.7$\times$ & $-$0.16 & $-$.0065 & +.0076 \\
& train      & 3.43 & 2.74 & 1.70 & 1.42 & 9.6$\times$ & $-$0.37 & $-$.0100 & +.0085 \\
\midrule
\multirow{2}{*}{\rotatebox{90}{\tiny DB}}
& drjohnson  & \textbf{7.90} & 7.13 & 7.14 & \textbf{8.30} & 9.3$\times$ & \textbf{$-$0.01} & $-$.0003 & +.0010 \\
& playroom   & 4.02 & 6.33 & 10.89 & 7.24 & 9.0$\times$ & \textbf{+0.04} & $-$.0017 & +.0009 \\
\midrule
\multicolumn{2}{l}{\textit{Overall (21 scenes)}} & \textit{2.99} & \textit{2.40} & \textit{3.01} & \textit{2.49} & \textit{9.1$\times$} & \textit{$-$0.32} & \textit{$-$.0068} & \textit{+.0065} \\
\bottomrule
\end{tabular}
\end{table*}
...
\addplot[only marks, mark=triangle*, mark size=3pt, color=red!70!black] coordinates {
    (5.23, -0.21) (1.69, -0.48) (0.53, -0.27) (2.38, -0.11)
    (3.97, -0.05) (2.44, -0.36) (0.38, -0.38) (5.46, -0.41) (1.88, -0.02)
};
...
\end{tabular}
\caption{\textbf{Visual quality comparison at $9.1{\times}$ average compression.} Left: original render. Center: \ours{} compressed. Right: error map (amplified $10{\times}$). \textit{drjohnson} (top): $0.01\dB$ loss---differences are imperceptible. \textit{garden} (middle): $0.05\dB$ loss. \textit{ficus} (bottom): $0.72\dB$ loss---worst case, with visible artifacts on fine leaf structures.}
\label{fig:renders}
\end{figure*}
