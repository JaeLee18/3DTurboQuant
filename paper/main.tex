...
\title{Spurious Harmonics: Overfitting Analysis and Training-Free\\Compression in 3D Gaussian Splatting}
...
\begin{abstract}
We show that spherical harmonic (SH) coefficients in 3D Gaussian Splatting (3DGS) massively overfit, contributing 2--7\,dB more to training views than to test views across 21 standard benchmarks.
...
Critically, on high-overfitting scenes, compression becomes effectively free: \textit{drjohnson} compresses $9.3{\times}$ with only $0.01\dB$ loss, and \textit{playroom} compresses $9.0{\times}$ while \emph{improving} quality by $0.04\dB$.
These results suggest the SH overfitting gap absorbs quantization damage.
...
Each primitive stores 59 floating-point parameters: a 3D position~(3), covariance via log-scale~(3) and rotation quaternion~(4), opacity~(1), and spherical harmonic~(SH) coefficients for view-dependent color.
...
By measuring the train/test PSNR gap across 21 scenes from four benchmarks (NeRF Synthetic~\cite{mildenhall2020nerf}, Mip\-NeRF360~\cite{barron2022mipnerf360}, Tanks \& Temples~\cite{knapitsch2017tanks}, Deep Blending~\cite{hedman2018deep}), we find gaps averaging $3.64\dB$ on NeRF Synthetic and reaching $7.90\dB$ on \textit{drjohnson}.
This overfitting is not uniform across the SH representation: it concentrates in high-frequency bands.
Band~$\ell{=}3$ exhibits an overfitting ratio $\R{3}{=}2.45$ on NeRF Synthetic, meaning it contributes $2.45{\times}$ more to fitting training views than to generalizing to test views.
This implies approximately $(\R{3}{-}1)/\R{3} \approx 59\%$ of band~$\ell{=}3$'s learned content is overfitting noise.
...
The resulting system, \ours{}, achieves $9.1{\times}$ compression at only $0.32\dB$ average quality loss, running entirely on CPU without any training, neural networks, or GPU.
Critically, on scenes with large overfitting gaps, compression is \emph{effectively free}: \textit{drjohnson} ($G{=}7.90\dB$) compresses $9.3{\times}$ with only $0.01\dB$ loss; \textit{playroom} ($G{=}4.02\dB$) compresses $9.0{\times}$ while \emph{improving} quality by $0.04\dB$.
The overfitting gap acts as a ``damage budget'' that absorbs quantization error before test-view quality is affected.

Our contributions are:
\begin{enumerate}
\item \textbf{First systematic quantification of SH overfitting} in full-view 3DGS across 21 scenes from 4 benchmarks, revealing 2--7\,dB train/test gaps and the consistent band-level hierarchy $\R{1} < \R{2} < \R{3}$.
\item \textbf{Discovery that compression becomes ``effectively free''} on high-overfitting scenes, where the overfitting gap absorbs quantization damage without degrading test quality.
...
Our work differs fundamentally: we provide \emph{provable optimality guarantees} on the quantization step and, more importantly, analyze \emph{why} compression works so well on certain scenes---namely, because SH overfitting provides a natural damage budget.
...
COLMAP-based scenes show even more striking overfitting.
The Deep Blending scenes are extreme: \textit{drjohnson} reaches a gap of $\mathbf{7.90}\dB$ and \textit{playroom} $4.02\dB$.
Among MipNeRF360 scenes, \textit{stump} ($5.46\dB$), \textit{bicycle} ($5.23\dB$), and \textit{garden} ($3.97\dB$) all show large gaps.

Two MipNeRF360 scenes (\textit{bonsai} at $-4.07\dB$ and \textit{room} at $-0.20\dB$) exhibit negative gaps where test PSNR exceeds train PSNR.
We attribute this to favorable test-view placement: the 1-in-8 holdout pattern in these scenes happens to select viewpoints with simpler content (less occlusion, fewer specular surfaces).
The existence of negative-gap scenes does not contradict the overfitting finding; it reflects the stochastic nature of train/test splits in COLMAP-based benchmarks.
...
Overfitting ratios on real-world scenes are even more dramatic.
\textit{Drjohnson} exhibits $\R{3}{=}8.30$: band~3 contributes $8.3{\times}$ more to training than test.
\textit{Stump} ($\R{3}{=}7.4$) and \textit{playroom} ($\R{3}{=}7.2$) are similarly extreme.
...
For \textit{drjohnson} ($\R{3}{=}8.30$), the noise fraction reaches ${\sim}88\%$.

\begin{table*}[t]
\centering
\caption{SH overfitting analysis and compression results across 21 benchmark scenes. \textbf{Left:} $G$~=~train/test PSNR gap (dB); $\R{k}$~=~overfitting ratio for SH band $\ell{=}k$ (Eq.~\ref{eq:overfitting_ratio}). Higher $\R{k}$ indicates more overfitting. The hierarchy $\R{1} < \R{2} < \R{3}$ holds on average. \textbf{Right:} \ours{} compression ratio and PSNR change ($\Delta$) at the balanced operating point. Bold entries highlight effectively free compression ($|\Delta| \leq 0.05\dB$).}
\label{tab:overfitting}
\small
\begin{tabular}{llrrrrrrr}
\toprule
& & \multicolumn{4}{c}{\textbf{Overfitting Analysis}} & \multicolumn{2}{c}{\textbf{\ours{} Compression}} \\
\cmidrule(lr){3-6} \cmidrule(lr){7-8}
\textbf{Dataset} & \textbf{Scene} & \textbf{$G$ (dB)} & $\boldsymbol{\R{1}}$ & $\boldsymbol{\R{2}}$ & $\boldsymbol{\R{3}}$ & \textbf{Ratio} & \textbf{$\Delta$PSNR} \\
\midrule
\multirow{8}{*}{\rotatebox{90}{\small NeRF Synthetic}}
& lego       & 3.95 & 1.72 & 2.04 & 2.29 & 8.7$\times$ & $-$0.17 \\
& chair      & 3.54 & 1.56 & 1.73 & 2.06 & 8.5$\times$ & $-$0.42 \\
& drums      & 3.47 & 2.83 & 3.59 & 3.55 & 8.6$\times$ & $-$0.19 \\
& ficus      & 3.32 & 1.64 & 1.47 & 3.28 & 8.6$\times$ & $-$0.72 \\
& hotdog     & 2.99 & 1.62 & 1.73 & 1.85 & 8.5$\times$ & $-$0.44 \\
& materials  & 5.14 & 1.75 & 2.29 & 2.38 & 8.6$\times$ & $-$0.21 \\
& mic        & 4.04 & 1.79 & 1.87 & 2.23 & 8.6$\times$ & $-$0.90 \\
& ship       & 2.68 & 1.38 & 1.71 & 1.97 & 8.8$\times$ & $-$0.39 \\
\cmidrule{2-8}
& \textit{Average} & \textit{3.64} & \textit{1.79} & \textit{2.06} & \textit{2.45} & \textit{8.6$\times$} & \textit{$-$0.43} \\
\midrule
\multirow{9}{*}{\rotatebox{90}{\small MipNeRF360}}
& bicycle    & 5.23 & 1.86 & 6.29 & 3.94 & 9.9$\times$ & $-$0.21 \\
& bonsai     & $-$4.07 & $-$0.73 & 0.41 & 0.12 & 9.5$\times$ & $-$0.48 \\
& counter    & 0.53 & 1.12 & 1.69 & 1.39 & 9.2$\times$ & $-$0.27 \\
& flowers    & 2.38 & 2.18 & 2.78 & 2.82 & 9.5$\times$ & $-$0.11 \\
& garden     & 3.97 & 1.82 & 2.90 & 2.80 & 9.2$\times$ & \textbf{$-$0.05} \\
& kitchen    & 2.44 & 1.94 & 1.95 & 1.50 & 9.3$\times$ & $-$0.36 \\
& room       & $-$0.20 & 1.26 & 1.13 & 0.98 & 9.9$\times$ & $-$0.38 \\
& stump      & 5.46 & 5.09 & 10.78 & 7.41 & 9.6$\times$ & $-$0.41 \\
& treehill   & 1.88 & 8.16 & 2.74 & $-$3.48 & 9.9$\times$ & \textbf{$-$0.02} \\
\cmidrule{2-8}
& \textit{Average} & \textit{1.96} & \textit{2.52} & \textit{3.41} & \textit{1.94} & \textit{9.6$\times$} & \textit{$-$0.25} \\
\midrule
\multirow{2}{*}{\rotatebox{90}{\tiny T\&T}}
& truck      & 1.51 & 1.13 & 1.57 & 1.30 & 9.7$\times$ & $-$0.16 \\
& train      & 3.43 & 2.74 & 1.70 & 1.42 & 9.6$\times$ & $-$0.37 \\
\midrule
\multirow{2}{*}{\rotatebox{90}{\tiny DB}}
& drjohnson  & \textbf{7.90} & 7.13 & 7.14 & \textbf{8.30} & 9.3$\times$ & \textbf{$-$0.01} \\
& playroom   & 4.02 & 6.33 & 10.89 & 7.24 & 9.0$\times$ & \textbf{+0.04} \\
\midrule
\multicolumn{2}{l}{\textit{Overall (21 scenes)}} & \textit{2.69} & \textit{2.40} & \textit{3.01} & \textit{2.49} & \textit{9.1$\times$} & \textit{$-$0.32} \\
\bottomrule
\end{tabular}
\end{table*}
...
The overfitting gap $G$ provides a direct interpretation as a \emph{damage budget} for compression.
If a model allocates $G\dB$ worth of its representational capacity to training-view-specific details, then up to $G\dB$ of compression-induced quality loss can be absorbed before test-view rendering degrades.
In practice, the relationship is not perfectly linear---compression damage does not target overfit content exclusively---but the correlation is striking.

Four scenes achieve what we term \emph{effectively free compression} (test PSNR loss $< 0.05\dB$ at ${\sim}9{\times}$ compression):
\begin{itemize}
  \item \textit{drjohnson} ($G{=}7.90\dB$): $9.3{\times}$ compression, $0.01\dB$ loss
  \item \textit{playroom} ($G{=}4.02\dB$): $9.0{\times}$, quality \emph{improves} by $0.04\dB$
  \item \textit{treehill} ($G{=}1.88\dB$): $9.9{\times}$, $0.02\dB$ loss
  \item \textit{garden} ($G{=}3.97\dB$): $9.2{\times}$, $0.05\dB$ loss
\end{itemize}
...
The quantization damage targets the overfit component rather than the generalizing signal.
In the extreme case of \textit{playroom}, compression acts as \emph{implicit regularization}, removing noise that slightly degraded test-view performance, leading to a net quality improvement.
...
On COLMAP-based scenes (MipNeRF360, T\&T, Deep Blending; 13 scenes), the average is $9.5{\times}$ at $0.23\dB$ loss---notably \emph{better} quality despite \emph{higher} compression ratios.
This counterintuitive result is explained by the overfitting analysis: COLMAP scenes have larger overfitting gaps (more noise to absorb), leading to less test-quality degradation per unit of compression.
...
\ours{} occupies a unique position in this design space along three axes:
(1)~it is the \emph{only} method with provable near-optimality guarantees (within $2.7{\times}$ of the Shannon bound on SH quantization);
(2)~it requires \emph{no GPU at any stage}---compression runs entirely on CPU;
and (3)~its information-theoretic grounding enables the overfitting analysis that constitutes the primary contribution of this paper.
For deployment scenarios requiring immediate compression of pre-trained models on CPU-only infrastructure, \ours{} defines the current Pareto frontier.
...
\toprule
\textbf{Method} & \textbf{Ratio} & \textbf{$\Delta$dB} & \textbf{Dev.} & \textbf{Tr.} & \textbf{Pr.} \\
\midrule
HAC++~\cite{chen2025hacpp}       & 100$\times$ & ${\sim}$0   & GPU & Y & -- \\
EntropyGS~\cite{entropygs2025}   & 30$\times$  & $-$0.04     & CPU & -- & -- \\
FlexGS~\cite{flexgaussian2025}   & 20$\times$  & $<$1.0      & GPU & -- & -- \\
FCGS~\cite{chen2024fcgs}         & 17$\times$  & $-$0.15     & GPU & -- & -- \\
LightGS~\cite{lightgaussian2024} & 15$\times$  & $<$0.5      & GPU & Y & -- \\
CompGS~\cite{compact3d2024}      & 10$\times$  & $<$0.5      & GPU & Y & -- \\
\midrule
\textbf{\ours{}} & \textbf{9.1$\times$} & \textbf{$-$0.32} & \textbf{CPU} & \textbf{--} & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
The most striking result is that \textit{playroom} achieves $9.0{\times}$ compression while \emph{improving} test PSNR by $0.04\dB$.
This is not a measurement artifact---it is reproduced consistently across multiple random seeds for the TurboQuant rotation matrix.

The mechanism is implicit regularization through quantization.
\textit{Playroom} has $\R{3}{=}7.24$, meaning band~3 contributes $7.24{\times}$ more to training than test---approximately $86\%$ of its content is noise.
When TurboQuant reduces band~3 precision from 32 to 2 bits, much of this noise is destroyed.
The small amount of genuine signal in band~3 is also partially corrupted, but the net effect is positive: removing $86\%$ noise at the cost of slightly degrading $14\%$ signal yields a quality improvement.
...
It suggests that standard 3DGS training over-allocates capacity to high-frequency SH bands, and that post-hoc quantization can partially correct this misallocation.
...
We presented the first systematic analysis of spherical harmonic overfitting in 3D Gaussian Splatting, revealing that SH band~$\ell{=}3$ is approximately 60\% noise across 21 standard benchmarks.
This finding reframes 3DGS compression: the majority of SH storage encodes overfitting artifacts, not useful signal, and removing it via quantization costs remarkably little in test-view quality.
On high-overfitting scenes, compression is effectively free---\textit{drjohnson} compresses $9.3{\times}$ at $0.01\dB$ loss, and \textit{playroom} actually improves under compression.
...
Our analysis of the quantization wall and per-band SH sensitivity establishes the rate-distortion frontier of training-free 3DGS compression.
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{Rate-distortion frontier of 3DGS compression.} \ours{} (blue curve) traces the Pareto frontier of training-free compression with provable optimality guarantees. Competing methods (individual markers) achieve higher ratios but require GPU access and/or training. The dotted vertical line marks the approximate quantization wall (${\sim}10{\times}$) beyond which training-free methods require Gaussian merging. The dashed horizontal line indicates zero quality loss.}
...
\addplot[only marks, mark=triangle*, mark size=3pt, color=red!70!black] coordinates {
    (5.23, -0.21) (-4.07, -0.48) (0.53, -0.27) (2.38, -0.11)
    (3.97, -0.05) (2.44, -0.36) (-0.20, -0.38) (5.46, -0.41) (1.88, -0.02)
};
...
\addplot[only marks, mark=square*, mark size=2.5pt, color=green!60!black] coordinates {
    (1.51, -0.16) (3.43, -0.37) (7.90, -0.01) (4.02, 0.04)
};
\addlegendentry{T\&T / Deep Blending}

% Labels for notable scenes
\node[font=\tiny, anchor=west] at (axis cs:7.90, -0.01) {\,drjohnson};
\node[font=\tiny, anchor=south west] at (axis cs:4.02, 0.04) {\,playroom};
\node[font=\tiny, anchor=south] at (axis cs:3.32, -0.72) {ficus};
\node[font=\tiny, anchor=south] at (axis cs:4.04, -0.90) {mic};
\node[font=\tiny, anchor=west] at (axis cs:1.88, -0.02) {\,treehill};

% Free compression line
\addplot[gray, dashed, line width=0.8pt] coordinates {(-5, 0) (9, 0)};
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{SH overfitting analysis.} (a)~Overfitting gap $G$ versus compression-induced PSNR change for all 21 scenes. High-gap scenes (drjohnson, playroom, treehill) cluster near zero compression loss, confirming the ``damage budget'' hypothesis. Scenes with genuine view-dependent content (mic, ficus) suffer more despite moderate gaps. (b)~Average overfitting ratio $\R{k}$ per SH band on NeRF Synthetic, showing the consistent hierarchy $\R{1} < \R{2} < \R{3}$. The dashed line at $\R{k}{=}1$ indicates zero overfitting; $\R{3}{=}2.45$ implies ${\sim}59\%$ of band~3 content is noise.}
